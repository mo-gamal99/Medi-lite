    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    {{-- Form Start --}}
                    <form action="{{route('currencies.update', $currentCurrency->id)}}" method="post"
                        ...
                        <div class="col-sm-10">
                            <select name="currency_id" id="currency" class="form-select">
                                ...
                                @foreach ($currencies as $currency)
                                    <option
                                        value="{{ $currency->id }}" @selected($currency->id == $currentCurrency->id)>{{ $currency->name_ar }}</option>
                                @endforeach
                            </select>
                            @error('currency_id')
                        ...
                        <div class="col-sm-12 mt-3">
                            ...
                            <div class="col-sm-10">
                                <x-form.input type="number" name="price_in_default_currency" step=".01"
                                              value="{{$currentCurrency->price_in_default_currency}}"
